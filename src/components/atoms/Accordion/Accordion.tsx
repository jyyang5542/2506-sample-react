...
const Accordion = ({ opened = false, title, contents, fontSize, px, pl, pr, py, pt, pb, mx, ml, mr, my, mt, mb }: IAccordion) => {
	...
	return (
		<Style.Wrap {...spacingProps}>
			<Style.Title onClick={handleAccordionToggle} opened={isAccordionOpen} fontSize={fontSize?.title}>
				<span>{title}</span>
				<Image src='https://www.iconpacks.net/icons/2/free-arrow-down-icon-3101-thumb.png' alt='arrow' width={16} height={16} />
			</Style.Title>
			{isAccordionOpen && <Style.Contents fontSize={fontSize?.contents}>{contents}</Style.Contents>}
		</Style.Wrap>
	);
};
